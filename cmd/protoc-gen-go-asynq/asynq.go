...
func (g *asynqGenerator) genServiceClient(service *protogen.Service, clientType string) {
	...
	/*
			type EventAsynqClientOptions struct {
				clientName string
		}

		type EventAsynqClientOption func(opts *EventAsynqClientOptions)

		func EventAsyncClientWithRedis(clientName string) EventAsynqClientOption {
				return func(opts *EventAsynqClientOptions) {
					if clientName != "" {
		...
		type EventAsynqClient struct {
				*asynq.Client
				options *EventAsynqClientOptions
		}

		var (
				eventAsynqClient     *EventAsynqClient
				eventAsynqClientOnce sync.Once
		)

		func NewEventAsynqClient(opts ...EventAsynqClientOption) *EventAsynqClient {
				eventAsynqClientOnce.Do(func() {
					options := &EventAsynqClientOptions{
						clientName: "default",
					}
					...
					bootstrap.AddBootstrap(eventAsynqClient)
				})
				return eventAsynqClient
		}
	*/
