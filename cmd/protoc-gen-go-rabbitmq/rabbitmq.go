...
const (
	...
	rabbitmqPackage      = protogen.GoImportPath("github.com/asjard/asjard/pkg/server/xrabbitmq")
	rabbitmqStorePackage = protogen.GoImportPath("github.com/asjard/asjard/pkg/stores/xrabbitmq")
...
func NewRabbitmqGenerator(plugin *protogen.Plugin, file *protogen.File) *rabbitmqGenerator {
	return &rabbitmqGenerator{
		plugin:     plugin,
		file:       file,
...
func (g *rabbitmqGenerator) Run() *protogen.GeneratedFile {
	if len(g.file.Services) == 0 {
		return nil
	}
	g.gen = g.plugin.NewGeneratedFile(g.file.GeneratedFilenamePrefix+"_rabbitmq.pb.go", g.file.GoImportPath)
...
func (g *rabbitmqGenerator) genFileContent() {
	for _, service := range g.file.Services {
		g.genService(service)
	}
}

func (g *rabbitmqGenerator) genService(service *protogen.Service) {
	handlerNames := make([]string, 0, len(service.Methods))
	serverType := service.GoName + "Server"
	clientType := service.GoName + "RabbitmqClient"
...
func (g *rabbitmqGenerator) genServiceDesc(service *protogen.Service, serverType string, handlerNames []string) {
	...
	// Service descriptor.
	g.gen.P("// ", serviceDescVar, " is the ", rabbitmqPackage.Ident("ServiceDesc"), " for ", service.GoName, " service.")
	g.gen.P("// It's only intended for direct use with ", rabbitmqPackage.Ident("AddHandler"), ",")
	g.gen.P("// and not to be introspected or modified (even as a copy)")
	g.genComment(service.Comments)
	g.gen.P("var ", serviceDescVar, " = ", rabbitmqPackage.Ident("ServiceDesc"), " {")
	g.gen.P("ServiceName: ", strconv.Quote(string(service.Desc.FullName())), ",")
	g.gen.P("HandlerType: (*", serverType, ")(nil),")
	g.gen.P("Methods: []", rabbitmqPackage.Ident("MethodDesc"), "{")
...
func (g *rabbitmqGenerator) genServiceClient(service *protogen.Service, clientType string) {
	...
	g.gen.P("*", rabbitmqStorePackage.Ident("ClientConn"))
	...
	g.gen.P("conn, err := ", rabbitmqStorePackage.Ident("Client"), "(", rabbitmqStorePackage.Ident("WithClientName"), "(c.options.clientName)", ")")
	g.gen.P("if err != nil {")
	g.gen.P("return err")
	g.gen.P("}")
...
func (g *rabbitmqGenerator) genServiceMethodClient(service *protogen.Service, clientType string, method *protogen.Method) {
...
func (g *rabbitmqGenerator) genServiceMethod(service *protogen.Service, serverType string, method *protogen.Method) string {
	hname := fmt.Sprintf("_%s_%s_RabbitmqHandler", service.GoName, method.GoName)
	g.genComment(method.Comments)
	g.gen.P("func ", hname, "(ctx *", rabbitmqPackage.Ident("Context"), ", srv any, interceptor ", serverPackage.Ident("UnaryServerInterceptor"), ") (any, error) {")
	...
	g.gen.P("Protocol: ", rabbitmqPackage.Ident("Protocol"), ",")
	g.gen.P("}")
...
func (g *rabbitmqGenerator) genLeadingComments(loc protoreflect.SourceLocation) {
...
func (g *rabbitmqGenerator) genComment(comments protogen.CommentSet) {
	if comments.Leading != "" {
		g.gen.P("//")
...
func (g *rabbitmqGenerator) protocVersion() string {
