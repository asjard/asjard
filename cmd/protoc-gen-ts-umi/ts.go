...
func (g *TsGenerator) Run() *protogen.GeneratedFile {
	if len(g.file.Services) == 0 || !g.hasHttpOption(g.file.Services) {
		return nil
	}
	filenamePrefix := strings.Join(strings.Split(g.file.GeneratedFilenamePrefix, "/")[1:], "/")
	g.gen = g.plugin.NewGeneratedFile(filenamePrefix+".umi.pb.ts", g.file.GoImportPath)
...
func (g *TsGenerator) hasHttpOption(services []*protogen.Service) bool {
