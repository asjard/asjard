...
func (g *TsGenerator) Run() *protogen.GeneratedFile {
	...
	g.gen = g.plugin.NewGeneratedFile(g.file.GeneratedFilenamePrefix+"_umi.pb.ts", g.file.GoImportPath)
...
func (g *TsGenerator) genServiceMethod(service *protogen.Service, method *protogen.Method) {
	...
	for _, httpOption := range httpOptions {
		...
		g.gen.P("export async function ", method.GoName, "(")
		...
		g.gen.P("){")
		g.gen.P("return request<{data:", method.Output.Desc.FullName(), "}>('", option.Path, "',{")
