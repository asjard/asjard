...
// Copy From: https://github.com/kubernetes/apimachinery/blob/master/pkg/util/rand/rand.go
...
const (
	...
	alphaUpper   = "BCDFGHJKLMNPQRSTVWXZ"
	alphaLower   = "bcdfghjklmnpqrstvwxz"
	alphaSpecial = "~!@#$%^&*()_+{}<>?"
	number       = "2456789"
...
func StringWithMode(n int, mode Mode) string {
	b := make([]byte, n)
	rng.Lock()
	defer rng.Unlock()
	chars := ""
	...
	if mode&AlphaUpper != 0 {
		chars += alphaUpper
	}
	if mode&AlphaLower != 0 {
		chars += alphaLower
	}
	if mode&AlphaSpecial != 0 {
		chars += alphaSpecial
	}
	if mode == 0 {
		chars = alphanums
	...
	for i := 0; i < n; {
		...
		if idx := int(randomInt63 & alphanumsIdxMask); idx < len(chars) {
			b[i] = chars[idx]
			i++
		}
		randomInt63 >>= alphanumsIdxBits
